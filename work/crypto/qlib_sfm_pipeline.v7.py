"""
qlib_sfm_pipeline.v7.py - SFM + Optuna + Top-K con mejoras sobre v6

Mejoras respecto a v6:
  1. Label a 5 dias (menos ruido que 1d)
  2. Features adicionales ademas de close/pct/ratio:
     - close, pct_change, ratio 5d (como v6)
     - volatilidad 20d rolling (desviacion estandar de retornos)
     - media movil 20d / close
     - rango diario (high-low)/close (simulado, usa close como proxy)
  3. Config rapida para pocas monedas (CRYPTO_INSTRUMENTS)

Arquitectura: SFMCellRefined con frecuencias adaptativas
Split: 60/20/20
Label: retorno a FWD_DAYS dias (default 5)
Denoising: OFF
...
def load_and_process_crypto_data(cryptos, start_date, end_date, fwd_days=5):
    """Carga cada cripto por separado con features extendidos y label a fwd_days."""
    print(f"   Cargando {len(cryptos)} criptos desde {start_date} hasta {end_date}...")
    print(f"   Label: retorno a {fwd_days} dias  |  Features: close, pct, ratio, vol, ma, rango")
    ...
    # Volatilidad 20d: desviacion estandar de retornos diarios en ventana 20d
    ...
    # Rango diario: usamos la variacion absoluta del close como proxy del rango
    # En datos reales seria (high-low)/close, pero solo tenemos close
    df_range = df_pct.abs()
...
def evaluate_top_k(study, k, cryptos, market_train_scaled, labels_train,
    ...
    for i, trial in enumerate(top_k):
        ...
        print(f"     #{i+1}: Sharpe_val={sharpe_val:.2f}  Sharpe_test={result['sharpe']:.2f}  Equity={result['equity_final']:.4f}x")
    ...
    print(f"\n   Top-{k}: Sharpe mu={stats['sharpe']['mean']:.2f}")
...
def plot_walk_forward(wf_results, output_dir):
    ...
    for i, r in enumerate(wf_results):
        ...
        ax.plot(x_days, sfm_curve, color=colors_sfm[i % len(colors_sfm)], linewidth=2.0, label='SFM')
        ax.plot(x_days, bm_curve, color='#888888', linestyle='--', linewidth=1.5, alpha=0.7, label='Baseline')
        ax.set_title(f'Ventana {r["window"]}  ({r["test_samples"]} muestras)', fontsize=12)
...
if __name__ == "__main__":
    ...
    FWD_DAYS = int(os.getenv("CRYPTO_FWD_DAYS", "5"))
    DO_WALK_FORWARD = False
    OUTPUT_DIR = os.getenv("CRYPTO_MODEL_OUTPUT_DIR",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "output/sfm_v7_label5d"))
    ...
    market_data, labels_data, dates_idx = load_and_process_crypto_data(CRYPTOS, START_DATE, END_DATE, fwd_days=FWD_DAYS)
    ...
    study = optuna.create_study(direction="minimize", sampler=TPESampler(seed=SEED),
                                ...
                                study_name="sfm_optuna_v7", storage=None)
    ...
    print(f"\nMEJOR TRIAL: Sharpe Val={best_sharpe_val:.2f}")
    ...
    n_pruned = sum(1 for t in study.trials if t.state == optuna.trial.TrialState.PRUNED)
    study_results = {
        "best_sharpe_val": best_sharpe_val, "fwd_days": FWD_DAYS,
        ...
        "split": "60/20/20", "lookback_range": "20-90", "features": "close,pct,ratio_5d,vol_20d,ma20_ratio,rango",
    ...
    # =================================================================
    print("\n" + "=" * 65)
    print(f"RESUMEN SFM v7 (label {FWD_DAYS}d)")
    print("=" * 65)
